        ...
        <div className="row" >
            ...
            <div className="col col2" style={{paddingBottom:'20px', paddingTop:'20px'}}>
                <div className="c2-content bg-secondary p-3 rounded shadow  ">
                    <h1 className='restaurant-id'>{restaurant.name}</h1>
                ...
                <div className="buttons">
                 <button onClick={handleShow} style={{width:'180px',margin:'15px 0 15px 0'}} className='btn btn-warning rounded'>
                    Operating Hours
                   </button>
                ...
                {restaurant.reviews && restaurant.reviews.length > 0 ? (
                  restaurant.reviews.map((review, index) => (
                    <div key={index} className='review-item'>
                      <h5 className='review-name'>{review.name} : {review.date}</h5>
                 
                      <p className='ratings'>Rating : {review.rating} Stars</p>
                      <p>{review.comments}</p>
                      <hr />
                    </div>
                  ))
            ...
            <p className='text-center' style={{paddingBottom:'10px' , fontSize:'1.2em'}}>Copyright © 2024 Lettuce EAT BISTRO. Built with React</p>
        </div>
        
        <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title><h1 className='operating-hours'>Operating Hours</h1></Modal.Title>
        </Modal.Header>
